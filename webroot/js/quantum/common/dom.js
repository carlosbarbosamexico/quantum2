...
Quantum.provide('Dom', {
  ...
  addCss: function(dom, className) {
    if (!Quantum.Dom.containsCss(dom, className)) {
  ...
  removeCss: function(dom, className) {
    if (Quantum.Dom.containsCss(dom, className)) {
      ...
      Quantum.Dom.removeCss(dom, className); // in case of repetition
    }
  },
  ...
  getStyle: function (dom, styleProp) {
    ...
    if (styleProp == 'opacity') {
      ...
      if (dom.currentStyle) { // camelCase (e.g. 'marginTop')
        Quantum.Array.forEach(styleProp.match(/\-([a-z])/g), function(match) {
  ...
  addCssRules: function(styles, names) {
    if (!Quantum.Dom._cssRules) {
      Quantum.Dom._cssRules = {};
    }

    // note, we potentially re-include CSS if it comes with other CSS that we
    // have previously not included.
    var allIncluded = true;
    Quantum.Array.forEach(names, function(id) {
      if (!(id in Quantum.Dom._cssRules)) {
        allIncluded = false;
        Quantum.Dom._cssRules[id] = true;
      }
    });
...
//#JSCOVERAGE_IF
    if (Quantum.Dom.getBrowserType() != 'ie') {
    ...
    if (!Quantum.Dom._browserType) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (userAgent.indexOf(keys[i]) >= 0) {
          Quantum.Dom._browserType = names[i];
          break;
        }
      }
    }
    return Quantum.Dom._browserType;
  },
   ...
   */
  ready: function(fn) {
    if (Quantum.Dom._isReady) {
      fn();
    } else {
      Quantum.Event.subscribe('dom.ready', fn);
    }
  }
});
...
(function() {
  ...
  function domReady() {
    Quantum.Dom._isReady = true;
    Quantum.Event.fire('dom.ready');
    Quantum.Event.clear('dom.ready');
  }

  // In case we're already ready.
  if (Quantum.Dom._isReady || document.readyState == 'complete') {
    return domReady();
  }
  ...
  if (Quantum.Dom.getBrowserType() == 'ie' && window === top) {
    (function() {
      try {
