 ...
 * NOTE: We tag this as Quantum.Event even though it is actually Quantum.EventProvider to
...
Quantum.provide('EventProvider', {
   ...
   * changes:
   *
   *     Quantum.Event.subscribe('auth.sessionChange', function(response) {
   ...
   * - xfbml.render -- fired when a call to Quantum.XQuantumML.parse() completes
  ...
  /**
   * Removes subscribers, inverse of [Quantum.Event.subscribe](Quantum.Event.subscribe).
   *
   ...
   * [Quantum.Event.subscribe](Quantum.event.subscribe), we get:
   *
   *     var onSessionChange = function(response) {
   *       // do something with response.session
   *     };
   *     Quantum.Event.subscribe('auth.sessionChange', onSessionChange);
   *
   *     // sometime later in your code you dont want to get notified anymore
   *     Quantum.Event.unsubscribe('auth.sessionChange', onSessionChange);
   *
   * @access public
   * @param name {String} Name of the event.
   * @param cb {Function} The handler function.
  ...
  unsubscribe: function(name, cb) {
    ...
    Quantum.Array.forEach(subs, function(value, key) {
      if (value == cb) {
        subs[key] = null;
      }
    });
   ...
   *
   * You need to pass the same event name that was passed to Quantum.Event.subscribe.
  ...
  fire: function() {
    ...
    Quantum.Array.forEach(this.subscribers()[name], function(sub) {
 ...
 *
 * @class Quantum.Event
 * @extends Quantum.EventProvider
 */
Quantum.provide('Event', Quantum.EventProvider);
