 ...
 * @provides fbp.anim
 * @layer basic
 * @requires fbp.prelude fbp.array fbp.dom
 */
...
Quantum.provide('Anim', {
  ...
  ate: function(dom, props, duration, callback) {
    ...
    var
      ...
      begin       = null,
      s           = dom.style,
      timer       = setInterval(Quantum.bind(function() {
        ...
        Quantum.Array.forEach(props, Quantum.bind(function(value, prop) {
          if (!from[prop]) { // parse from CSS
            var style = Quantum.Dom.getStyle(dom, prop);
          ...
          Quantum.Array.forEach(from[prop], function(pair, i) {
          ...
          Quantum.Dom.setStyle(dom, prop, next);
        }, this));
  ...
  _parseCSS: function(css) {
    var ret = [];
    Quantum.Array.forEach(css.split(' '), function(peice) {
